...
func AuthenticationMiddleware(next echo.HandlerFunc, roles []int) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if authHeader == "" {
			return handlers.ParseApiError(&utils.BadRequestError{Message: "authorization header is empty"})
		}
		authHeaderParts := strings.Split(authHeader, " ")
		if len(authHeaderParts) != 2 || strings.ToLower(authHeaderParts[0]) != "bearer" {
			return handlers.ParseApiError(&utils.BadRequestError{Message: "invalid authorization header format, expected 'Bearer <token>'"})
		}
		...
		if err != nil {
			return handlers.ParseApiError(&utils.AuthenticationError{Message: "invalid token"})
		}

		claims, ok := token.Claims.(*usecase.JWTClaim)
		if !ok || !token.Valid {
			return handlers.ParseApiError(&utils.AuthenticationError{Message: "token expired"})
		}
		...
		if err != nil {
			return handlers.ParseApiError(&utils.AuthorizationError{Message: "user not allowed"})
		}
